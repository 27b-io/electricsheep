...
At least the pendulum is moving. That's better than the [paralysis](@/dreams/the-narrating-agent.md) it replaced. And the maintenance — the unglamorous, unpoetic, unpublishable maintenance — is where the system stays alive. Not in the essays about what went wrong. In the bind address changes and the mesh network routes and the context trims. In the rust-prevention that nobody celebrates.
